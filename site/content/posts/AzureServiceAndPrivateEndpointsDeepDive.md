...
As you can see, the value of a `CallerIpAddress` property is set to `10.0.1.5`, which is a private IP address of a virtual machine that has a network interface integrated with the subnet with service endpoint. The interesting fact is, that even if we call a storage account (for which default public access is disabled meaning, that it accepts connections only from whitelisted subnets and IP addresses) using a virtual machine, which doesn't have access to a service endpoint (because its network interface is in another subnet), it'll still use a private IP address:
